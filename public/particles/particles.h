...
{
    CParticleControlPoint() :  // явный конструктор вместо инициализации в объ€влении
        m_Position(vec3_origin),
        m_PrevPosition(vec3_origin),
        m_ForwardVector(vec3_origin),
        m_UpVector(vec3_origin),
        m_RightVector(vec3_origin),
