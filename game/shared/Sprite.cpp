...
BEGIN_DATADESC( CSprite )
	...
	DEFINE_KEYFIELD( m_flMaxFrame, FIELD_FLOAT, "frame" ),
#ifdef PORTAL
...
void CSprite::Spawn( void )
{
	...
	float frame = (float)modelinfo->GetModelFrameCount( GetModel() ) - 1;
	if( frame > 0.0 )
		m_flMaxFrame = frame;
...
#if !defined( CLIENT_DLL )
	if( GetEntityName() == AllocPooledString( "hl2mp.ru-healthbar" ) ) {

		CBaseEntity *pRecipientEntity = CBaseEntity::Instance( pInfo->m_pClientEnt );
		if( !pRecipientEntity || GetOwnerEntity() != pRecipientEntity )
			return FL_EDICT_DONTSEND;

		if( GetMoveParent() ) {
			CBaseEntity *pEntity = GetMoveParent();
			if( pEntity->IsNPC() && pEntity->IsAlive() ) {
				CAI_BaseNPC *pNpc = dynamic_cast<CAI_BaseNPC *>( pEntity );
				SetAbsOrigin( pNpc->GetAbsOrigin() + Vector(0,0,pNpc->GetHullHeight()) );
			}
			else {
				SetAbsOrigin( pEntity->EyePosition() + Vector( 0, 0, 10.0 ) );
			}
