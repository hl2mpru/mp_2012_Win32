...
void CHL2MPRules::Think( void )
{
	...
	else if( engine->GetEntityCount() >= 1900 ) {
		UTIL_ClientPrintAll( HUD_PRINTCENTER,  UTIL_VarArgs( "WARNING: Entity stack overflow: %d/1950", engine->GetEntityCount() ) );
	}

	//D@Ni1986: Нужно переписать think в самой ентити
...
{
	if ( weaponstay.GetInt() > 0 && pItem->HasSpawnFlags( SF_NORESPAWN ) == false )
	{
		if ( pPlayer->Weapon_OwnsThisType( pItem->GetClassname(), pItem->GetSubType() ) && !FClassnameIs( pItem, "weapon_medkit") && !FClassnameIs( pItem, "weapon_frag") && !FClassnameIs( pItem, "weapon_slam") ) {
			return false;
		}
	}
...
ConVar mp_noblock("mp_noblock", "1", FCVAR_REPLICATED);
...
bool CHL2MPRules::ShouldCollide(int collisionGroup0, int collisionGroup1)
{
	...
	//trigger_changelevel
	if (collisionGroup0 == 40 || collisionGroup1 == 40)
	{
		if (collisionGroup0 == COLLISION_GROUP_VEHICLE || collisionGroup0 == COLLISION_GROUP_PLAYER)
			return true;
		return false;
	}

	if( collisionGroup0 == COLLISION_GROUP_PLAYER && collisionGroup1 == COLLISION_GROUP_PLAYER )
			return !mp_noblock.GetBool();
...
CAmmoDef *GetAmmoDef()
{
	...
	if (!bInitted)
	{
		...
		//def.AddAmmoType("Battery",			DMG_CLUB,					TRACER_NONE,			NULL, NULL, NULL, 0.0, 0, 4, 8 );
		def.AddAmmoType("GaussEnergy",		DMG_SHOCK,					TRACER_NONE,			"sk_jeep_gauss_damage",		"sk_jeep_gauss_damage", "sk_max_gauss_round", BULLET_IMPULSE(650, 8000), 0 ); // hit like a 10kg weight at 400 in/s
		def.AddAmmoType("CombineCannon",	DMG_BULLET,					TRACER_LINE,			"sk_npc_dmg_gunship_to_plr", "sk_npc_dmg_gunship", NULL, 1.5 * 750 * 12, 0 ); // hit like a 1.5kg weight at 750 ft/s
		def.AddAmmoType("AirboatGun",		DMG_AIRBOAT,				TRACER_LINE,			"sk_plr_dmg_airboat",		"sk_npc_dmg_airboat",		"sk_airboat_max_ammo",					BULLET_IMPULSE(10, 600), 0 );
		...
		def.AddAmmoType("HealthAmmo", DMG_GENERIC, TRACER_NONE, 0, 0, 40, 0, 0);
	}
