	...
	default:
		{
			if ( (pField->fieldSizeInBytes != pField->fieldSize * gSizes[pField->fieldType]) )
			{
				Warning("WARNING! Field %d %d %d %d %s is using the wrong FIELD_ type!\nFix this or you'll see a crash.\n", pField->fieldSizeInBytes, pField->fieldSize, gSizes[pField->fieldType], pField->fieldType, pField->fieldName );
				Assert( 0 );
			}
...
void CRestore::EmptyFields( void *pBaseData, typedescription_t *pFields, int fieldCount )
{
	...
	for ( i = 0; i < fieldCount; i++ )
	{
		...
		default:
			...
			if ( pField->fieldSizeInBytes != pField->fieldSize * gSizes[pField->fieldType] )
			{
				Warning("WARNING EmptyFields()! Field %s is using the wrong FIELD_ type!\nFix this or you'll see a crash.\n", pField->fieldName );
				Assert( 0 );
			}
...
//hl2mp.ru - LINUX ALLOC
extern void *SaveAllocMemory( size_t num, size_t size, bool bClear = false );
...
void CEntitySaveRestoreBlockHandler::ReadRestoreHeaders( IRestore *pRestore )
{
	...
	//hl2mp.ru - LINUX ALLOC
	entitytable_t *pEntityTable = ( entitytable_t *)SaveAllocMemory( (sizeof(entitytable_t) * nEntities), sizeof(char) );
	if ( !pEntityTable )
	{
		return;
	}
...
bool CEntitySaveRestoreBlockHandler::SaveInitEntities( CSaveRestoreData *pSaveData )
{
	...
	//hl2mp.ru - LINUX ALLOC
	entitytable_t *pEntityTable = ( entitytable_t *)SaveAllocMemory( (sizeof(entitytable_t) * number_of_entities), sizeof(char) );
	if ( !pEntityTable )
		return false;
...
void CreateEntitiesInTransitionList( CSaveRestoreData *pSaveData, int levelMask )
{
	...
	for ( i = 0; i < pSaveData->NumEntities(); i++ )
	{
		...
		if ( pEntInfo->classname == NULL_STRING )
		{
			Msg( "Entity with data saved, but with no classname\n" );
			Assert(0);
			continue;
		}
		...
		{
			//hl2mp.ru - Не нужно в мультиплеере
			/*
			edict_t *ed = INDEXENT( pEntInfo->edictindex );
