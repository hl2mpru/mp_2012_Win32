	...
	if ( MessageToAll() )
	{
		for( int i=1; i<=gpGlobals->maxClients; i++ )
		{
			...
			if( player && !player->IsFakeClient() ) {
				...
				msg = UTIL_TranslateMsg( szLanguage, msg );
				UTIL_HudMessage( player, m_textParms, msg );
			}
		}
...
private:
	...
	void		Equip( CBasePlayer *pPlayer, const char *szValue );
...
BEGIN_DATADESC( CGamePlayerEquip )
	...
	DEFINE_INPUTFUNC( FIELD_STRING, "EquipAllPlayers", EquipAll ),
	DEFINE_INPUTFUNC( FIELD_VOID, "Enable", InputEnable ),
	DEFINE_INPUTFUNC( FIELD_VOID, "Disable", InputDisable ),

END_DATADESC()

void CGamePlayerEquip::InputEnable( inputdata_t &inputdata )
{
	RemoveSpawnFlags( SF_DISABLED );
	m_iDisabled = FALSE;
}

void CGamePlayerEquip::InputDisable( inputdata_t &inputdata )
{
	AddSpawnFlags( SF_DISABLED );
	m_iDisabled = TRUE;
}
...
void CGamePlayerEquip::EquipActivator( inputdata_t &inputData )
{
	...
	if( pActivator && pActivator->IsPlayer() ) {
		...
		if( pPlayer )
			Equip( pPlayer, inputData.value.String() );
...
void CGamePlayerEquip::EquipPlayer( CBaseEntity *pEntity )
{
	...
	for ( int i = 0; i < MAX_EQUIP; i++ )
	{
		...
		for ( int j = 0; j < m_weaponCount[i]; j++ )
			Equip( pPlayer, STRING(m_weaponNames[i]) );
	}
}

void CGamePlayerEquip::Equip( CBasePlayer *pPlayer, const char *szValue )
{
	if ( strncmp( szValue, "ammo_", 5 ) == 0 ) {
		szValue += 5;
		ITEM_GiveAmmo( pPlayer, 1, szValue, true );
		return;
	}

	if( strcmpi( szValue, "weapon_medkit" ) != 0 )
				pPlayer->EquipSuit();

 	CBaseEntity *equip = pPlayer->GiveNamedItem( szValue );
