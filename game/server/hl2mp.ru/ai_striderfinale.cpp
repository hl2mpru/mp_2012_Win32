...
void CAI_StriderFinale::MovementThink()
{
	...
	if ( m_npcStrider1 )
	{
		CBasePlayer *pPlayer = UTIL_GetNearestPlayer( m_npcStrider1->GetAbsOrigin() );
...
void CAI_StriderFinale::FillRoles()
{
	...
	if ( m_npcStrider1 && m_npcStrider2 && bUpdated )
	{
		...
		if ( pPlayer && m_npcStrider1->StriderEnemyDistance(pPlayer) > m_npcStrider2->StriderEnemyDistance(pPlayer) )
		{
