...
extern bool ExtraChatCommand( CBasePlayer *pPlayer, char *msg );
...
void Host_Say( edict_t *pEdict, const CCommand &args, bool teamonly )
{
	...
	if( pPlayer && ExtraChatCommand( pPlayer, p ) )
		return;
...
void EnableNoClip( CBasePlayer *pPlayer )
{
	...
	ClientPrint( pPlayer, HUD_PRINTTALK, "noclip ON") ;
...
void CC_Player_NoClip( void )
{
	...
	ClientPrint( pPlayer, HUD_PRINTTALK, "noclip OFF" );
	if ( !TestEntityPosition( pPlayer ) )
	{
