...
void CNPC_CombineGunship::FireCannonRound( void )
{
	...
	else
	{
		...
		//hl2mp.ru - в кооп режиме это не нужно!
...
int	CNPC_CombineGunship::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if ( m_bInvulnerable == false )
	{
		...
		//hl2mp.ru - Автобаланс
		int playersOnline = UTIL_GetPlayerOnline();
		iHealthIncrements = floor(iHealthIncrements * (float)playersOnline);
