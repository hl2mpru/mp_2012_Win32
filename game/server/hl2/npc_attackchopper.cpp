...
private:
	...
	bool IsValidEnemy( CBaseEntity *pEnemy ) {
		//Вертолет будет игнорировать ракеты
		if ( pEnemy->IsAlive() && FClassnameIs( pEnemy, "rpg_missile" ) )
			return false;
...
bool CNPC_AttackHelicopter::DoGunCharging( )
{
	...
	case SHOOT_MODE_FAST:
		{
			//hl2mp.ru - Делаем баланс, количество выстрелов по умолчанию, умножаем на количество игроков онлайн
			int nBurstCount = sk_helicopter_burstcount.GetInt() * UTIL_GetPlayerOnline();
...
int CNPC_AttackHelicopter::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( pGren && info.GetAttacker() && info.GetAttacker()->IsPlayer() )
	{
		...
		if( g_pGameRules->IsSkillLevel(SKILL_EASY) )
		{
			damage = sk_helicopter_num_bombs1.GetFloat();
		}
		else if( g_pGameRules->IsSkillLevel(SKILL_HARD) )
		{
			damage = sk_helicopter_num_bombs3.GetFloat();
		}
		else // Medium, or unspecified
		{
			damage = sk_helicopter_num_bombs2.GetFloat();
		}
		damage = GetMaxHealth() / damage * UTIL_GetPlayerOnline();
...
//hl2mp.ru - От ракет не будет отбрасывать
