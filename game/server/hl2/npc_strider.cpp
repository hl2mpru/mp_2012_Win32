	...
	if ( (info.GetDamageType() & DMG_BLAST) && info.GetMaxDamage() > 50 )
	{
		...
		if ( dist < 200 )
		{
			...
			if( bPlayer )
			{
				...
				if( g_pGameRules->IsSkillLevel(SKILL_EASY) )
				{
					numMissiles = sk_strider_num_missiles1.GetFloat();
				}
				else if( g_pGameRules->IsSkillLevel(SKILL_HARD) )
				{
					numMissiles = sk_strider_num_missiles3.GetFloat();
				}
				else // Medium, or unspecified
				{
					numMissiles = sk_strider_num_missiles2.GetFloat();
				}
				damage = GetMaxHealth() / ( numMissiles * UTIL_GetPlayerOnline() );
				//hl2mp.ru - Автобаланс конец
			}

			CBasePlayer *pPlayer = ToBasePlayer( info.GetAttacker() );
			if( pPlayer ) {
				static int m_hTestPL[MAX_PLAYERS];
				++m_hTestPL[pPlayer->entindex()];
				if( !GetPlayerMP() && m_hTestPL[pPlayer->entindex()] >= 4 ) {
					pPlayer->IncrementFragCount( 1 );
					pPlayer->AddPointsToTeam( 1, false );
					m_hTestPL[pPlayer->entindex()] = 0;
				}
			}
