...
//D@Ni1986: Не дописана функция
int UTIL_GetPlayerOnline(CBaseEntity* pVisible, bool onlyVisible) {
	int totalPlayers = 0;
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if (pVisible && pVisible->FVisible(pPlayer, MASK_SHOT) == false)
			continue;

		if (onlyVisible) {
			for (int p = 1; p <= gpGlobals->maxClients; p++)
			{
				CBasePlayer* pVisiblePlayer = UTIL_PlayerByIndex(p);
				if (pPlayer != pVisiblePlayer && pPlayer->FVisible(pVisiblePlayer, MASK_SHOT) == false)
					continue;
			}
		}
...
void UTIL_SayText( const char *pText, CBasePlayer *pToPlayer )
{
	...
	pText = UTIL_TranslateMsg( engine->GetClientConVarValue( ENTINDEX( pToPlayer ), "cl_language" ), pText );
...
void UTIL_SetModel( CBaseEntity *pEntity, const char *pModelName )
{
	...
	if( !engine->IsModelPrecached( pModelName ) )
...
{
	//hl2mp.ru - При каждом новом вызове функции проверяется следующий клиент в ПВС а не любой доступный
	CBaseEntity *pe = GetContainingEntity( pEdict );
	if( pe ) {
		CBasePlayer *pPlayer = UTIL_GetNearestPlayer( pe->GetAbsOrigin() );
		if( pPlayer )
...
edict_t *UTIL_FindClientInVisibilityPVS( edict_t *pEdict )
{
	...
	CBaseEntity *pe = GetContainingEntity( pEdict );
	if( pe ) {
		CBasePlayer *pPlayer = UTIL_GetNearestVisiblePlayer( pe );
		if( pPlayer )
			return pPlayer->edict();
	}
